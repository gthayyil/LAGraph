...
def hdip_fiedler(L,lnorm,kmax =[20,50],emax = .000001,tol= .000001 ):
    """
    ...
    for i in range(1,kmax[0]+1):
        ...
        normer=y-lamb*x
        e=(normer.apply(gb.types.FP32.ABS)).max()
        ...
        last_err=err
        #x(1:n) = L2 \ x(1:n)
        my=mypcg2(L,u,alpha,indiag,x,tol,kmax[1])
        x=my[0]
        kk=my[1]
        k_inner = k_inner+kk
        x[0]=0
    #compute beta = u'*x/alpha
    beta = (u.emult(x).reduce_float(mon=gb.types.FP32.PLUS_MONOID))/alpha
    #set x = x - beta*u
    x=x-beta*u
